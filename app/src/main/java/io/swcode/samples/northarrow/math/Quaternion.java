...
import static io.swcode.samples.northarrow.math.MathUtilKt.radianToDegree;
...
public class Quaternion {
  ...
  public static float getAngle(Quaternion quaternion) {
    if (quaternion.w > 1) {
      quaternion.normalize(); // if w>1 acos and sqrt will produce errors, this cant happen if quaternion is normalised
    }

    return (float) (2 * Math.acos(quaternion.w));
  }

  public static float getAngleDegree(Quaternion quaternion) {
    return (float) radianToDegree(Quaternion.getAngle(quaternion));
  }
